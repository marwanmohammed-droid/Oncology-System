...
const ONGOING_MEDICATIONS = [
  ...
  'Statins', 'Diuretics', 'Beta-blockers', 'Corticosteroids', 'NSAIDs / Analgesics',
  'PPIs (Proton Pump Inhibitors)', 'Thyroid medications', 'Antidepressants', 'Antipsychotics',
  'Immunosuppressants', 'Bronchodilators / Inhalers',
]
...
export function Step2Medical({ onSave, saving, error, patientSex }: Props) {
  ...
  const primarySite1 = watch('diagnosis.primary_site')
  const primarySite2 = watch('diagnosis.primary_site_2')

  function getHistologyOptionsFor(site: string | undefined): string[] {
    if (!site || !PRIMARY_SITE_HISTOLOGY_MAP[site]) {
      // لو العضو مش محدد أو مش موجود في الخريطة، استخدم القايمة الكاملة
      return allHistologyTypes
    }
    // القايمة الخاصة بالعضو + أي أنواع مخصصة اتضافت يدويًا (تفضل متاحة دايمًا في البحث)
    return Array.from(new Set([...PRIMARY_SITE_HISTOLOGY_MAP[site], ...customHistologies.map(t => t.name)]))
  }

  const histologyOptions1 = getHistologyOptionsFor(primarySite1)
  const histologyOptions2 = getHistologyOptionsFor(primarySite2)
  ...
  function toggleMedication(item: string) {
    setSelectedMedications(prev => prev.includes(item) ? prev.filter(m => m !== item) : [...prev, item])
  }
  ...
  function addOncologyFH() {
    setOncologyFhEntries(prev => [...prev, { id: Math.random().toString(36).slice(2), person: '', type: '' }])
  }
  function updateOncologyFH(id: string, field: keyof OncologyFHEntry, value: string) {
    setOncologyFhEntries(prev => prev.map(e => e.id === id ? { ...e, [field]: value } : e))
  }
  function removeOncologyFH(id: string) {
    setOncologyFhEntries(prev => prev.filter(e => e.id !== id))
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" dir="ltr">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex gap-2 text-sm text-amber-700">
        <span>⚠️</span>
        <span>All clinical data must be entered in English only — SNOMED, HL7 compliance required.</span>
      </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              ...
              <div>
                ...
                <SearchableSelect
                  ...
                  options={histologyOptions1}
                  placeholder="Search or select histology..."
                  addNewLabel="Add new histology"
                  onAddNew={async (name) => { await addCustomHistology({ name }) }}
                />
                {primarySite1 && PRIMARY_SITE_HISTOLOGY_MAP[primarySite1] && (
                  <p style={{ fontSize: 9, color: '#8e97b5', marginTop: 4 }}>
                    عرض الأنواع الخاصة بـ {primarySite1} — أو ابحث لإضافة نوع آخر
                  </p>
                )}
              </div>
            </div>
